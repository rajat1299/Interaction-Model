...
- **Runtime:** Python 3.12, managed with `uv`. Deps: `pydantic` v2, `fastapi` + `uvicorn` (WebSocket transport), `httpx` (0b provider calls), `python-dotenv` (local secret loading), `pytest` + `pytest-asyncio`, `hypothesis` (property tests), `ruff`. Nothing else without a written justification in the PR.
...
interactionmodel/
  ...
  src/im/
    ...
    policy/
      ...
      prompted.py      # OpenAI frontier policy (WP13)
  client/
    index.html
...
`policy/prompted.py`: OpenAI Responses client (httpx), pinned to `gpt-5.6-terra` with high reasoning by the WP12 calibration pilot. Prompt = system: behavior spec + action JSON Schema + "emit exactly one action object"; user: policy stream bytes. OpenAI strict Structured Outputs cannot accept the frozen bare root union (`oneOf` plus conditional composition), so the existing provider-fallback path is used: JSON mode followed by authoritative Pydantic validation and one corrective retry on invalid. This preserves the raw bare action rather than adding a transport wrapper. Temperature is omitted because this is a reasoning-model Responses request. Model id, reasoning effort, output cap, and endpoint are separate policy config fields. Exact request/response bytes, latency, outcome, and prompt hash are stored in the operational audit lane. The same request builder emits Batch JSONL and offline expected/conservative cost estimates. Record: this 0b prompt renderer is *not* the Phase-3 training renderer; only the stream serialization is shared.

Acceptance: mocked-transport unit tests (valid, invalid-then-retry, refusal); dry-run configuration and cost report make no API call; one recorded live end-to-end session in the browser harness with the frontier policy driving mark + timer + cancel on real typing (manual run, transcript committed to `probes/results/e2e/`).
